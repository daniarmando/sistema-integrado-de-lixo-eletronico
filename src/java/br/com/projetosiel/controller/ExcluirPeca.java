 ...
 * @author Daniel
 */
public class ExcluirPeca extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            PecaEquipamentoDAOImpl daoPecaEquipamento = new PecaEquipamentoDAOImpl();
            daoPecaEquipamento.excluir(codigoPeca);
            
            GenericDAO dao = new PecaDAOImpl();
            dao.excluir(codigoPeca);
            
            request.getRequestDispatcher("ListarPeca").forward(request, response); 
        }catch (Exception ex){
            System.out.println("Problemas no servlet ao excluir Peça! Erro: "+ex.getMessage());
            ex.printStackTrace();
        }
    }
