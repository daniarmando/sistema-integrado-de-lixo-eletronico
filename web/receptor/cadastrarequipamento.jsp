<%-- 
    ...
    Created on : 28/05/2013, 20:13:22
    Author     : Daniel
--%>
...
<div id="content">
    ...
    <select name="idcategoria" class="select">
		  ...
		  <label for="equipamento">Nome do Equipamento</label>
                  ...
                  <td><select name="nomeequipamento" class="select">
              ...
              <option value="<%= listaEquipamento.getNomeListaEquipamento() %>"><%= listaEquipamento.getNomeListaEquipamento() %></option>
...
<table id="list" class="zebra">
    ...
    <tr>
        <td>&nbsp;</td>        
        <td colspan="2">OBs. Selecione as peças funcionais do equipamento, você pode descrever a capacidade em detalhe (ex. 2gb) e após terminar clique em cadastrar.</td>
    </tr>
    </tfoot>    
            <tbody>
                <%
                List <ListaPeca> listapecas = (List<ListaPeca>)request.getAttribute("listapecas");
                for (ListaPeca listaPeca : listapecas){
                %>
                <tr>
                    <td><input type="checkbox" name="<%= listaPeca.getNomeListaPeca() %>"/><%= listaPeca.getNomeListaPeca() %></td>
                    <td><input type="text" name="detalhepeca" size="2px"/></td>
